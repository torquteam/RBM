...
print("Current Working irectory:", current_directory)
...
num_basis_states_f = [3,3,4,3,3,4,3,3,3,4,2,3,3,3]
num_basis_states_g = [3,3,4,3,3,4,3,3,3,4,2,3,3,3]
num_basis_states_c = [3,3,4,3,3,4,3,4,3,4,2]
num_basis_states_d = [3,3,4,3,3,4,3,4,3,4,2]
num_basis_states_s = 9
num_basis_states_v = 9
num_basis_states_b = 9
num_basis_states_a = 4
mNuc_mev = 939
mOmega = 782.5
mRho = 763.0

# Define parameters of model
mSigma, gs2, gw2, gp2, kappa, lambda0, zeta, lambdav = sp.symbols('mSigma gs2 gw2 gp2 kappa lambda0 zeta lambdav')
param_list = [mSigma,gs2,gw2,gp2,kappa,lambda0,zeta,lambdav]
##################################################################
...
S_basis = func.perform_pod(sigma_fields, num_basis_states_s)
V_basis = func.perform_pod(omega_fields, num_basis_states_v)
B_basis = func.perform_pod(rho_fields, num_basis_states_b)
A_basis = func.perform_pod(coulomb_fields, num_basis_states_a)
f_basis = [func.perform_pod(f_fields[i], num_basis_states_f[i]) for i in range(nstates_n)]
g_basis = [func.perform_pod(g_fields[i], num_basis_states_g[i]) for i in range(nstates_n)]
c_basis = [func.perform_pod(c_fields[i], num_basis_states_c[i]) for i in range(nstates_p)]
d_basis = [func.perform_pod(d_fields[i], num_basis_states_d[i]) for i in range(nstates_p)]
...
f_basis = [f_basis[i]*np.mean(f_fields[0][0,:])/f_basis[0][0,0] for i in range(nstates_n)]
g_basis = [g_basis[i]*np.mean(g_fields[0][0,:])/g_basis[0][0,0] for i in range(nstates_n)]
c_basis = [c_basis[i]*np.mean(c_fields[0][0,:])/c_basis[0][0,0] for i in range(nstates_p)]
d_basis = [d_basis[i]*np.mean(d_fields[0][0,:])/d_basis[0][0,0] for i in range(nstates_p)]
...
for i in range(nstates_n):
    for j in range(num_basis_states_g[i]):
        galerk_g_j = func.wf_project(i,j,g_basis,coeff_set_g,dgdr_basis,r_vec) + state_info_n[i][1]*func.wf_project(i,j,g_basis,coeff_set_g,func.divide_array_wf(g_basis,r_vec),r_vec) \
                ...
                + conv_r0_en*func.meson_wf_project(i,j,g_basis,coeff_set_f,coeff_set_s,f_basis,S_basis,r_vec)
...
for i in range(nstates_n):
    for j in range(num_basis_states_f[i]):
        galerk_f_j = func.wf_project(i,j,f_basis,coeff_set_f,dfdr_basis,r_vec) - state_info_n[i][1]*func.wf_project(i,j,f_basis,coeff_set_f,func.divide_array_wf(f_basis,r_vec),r_vec) \
                ...
                + conv_r0_en*func.meson_wf_project(i,j,f_basis,coeff_set_g,coeff_set_s,g_basis,S_basis,r_vec)
...
galerk_s, galerk_v, galerk_b, galerk_a = ([] for i in range(4))
...
equations_f, equations_g, equations_c, equations_d, equations_s, equations_v, equations_b, equations_a = ([] for _ in range(8))
...
all_symbols = all_wf_symbols + coeff_set_s + coeff_set_v + coeff_set_b + coeff_set_a + en_set_n + en_set_p
#print(all_symbols)
...
######################################################################################
# sigma terms
sdensn_s = func.replace_coeff_with_vector(sdensn_s,all_symbols)
sdensp_s = func.replace_coeff_with_vector(sdensp_s,all_symbols)
dsdr2_s = func.replace_coeff_with_vector(dsdr2_s,all_symbols)
dsdr_s = func.replace_coeff_with_vector(dsdr_s,all_symbols)
S_s = func.replace_coeff_with_vector(S_s,all_symbols)
S2_s = func.replace_coeff_with_vector(S2_s,all_symbols)
S3_s = func.replace_coeff_with_vector(S3_s,all_symbols)

# omega terms
vdensn_v = func.replace_coeff_with_vector(vdensn_v,all_symbols)
vdensp_v = func.replace_coeff_with_vector(vdensp_v,all_symbols)
dvdr2_v = func.replace_coeff_with_vector(dvdr2_v,all_symbols)
dvdr_v = func.replace_coeff_with_vector(dvdr_v,all_symbols)
V_v = func.replace_coeff_with_vector(V_v,all_symbols)
V3_v = func.replace_coeff_with_vector(V3_v,all_symbols)
VB2_v = func.replace_coeff_with_vector(VB2_v,all_symbols)

# rho terms
vdensn_b = func.replace_coeff_with_vector(vdensn_b,all_symbols)
vdensp_b = func.replace_coeff_with_vector(vdensp_b,all_symbols)
dbdr2_b = func.replace_coeff_with_vector(dbdr2_b,all_symbols)
dbdr_b = func.replace_coeff_with_vector(dbdr_b,all_symbols)
B_b = func.replace_coeff_with_vector(B_b,all_symbols)
BV2_b = func.replace_coeff_with_vector(BV2_b,all_symbols)

# coulomb terms
vdensp_a = func.replace_coeff_with_vector(vdensp_a,all_symbols)
dadr2_a = func.replace_coeff_with_vector(dadr2_a,all_symbols)
dadr_a = func.replace_coeff_with_vector(dadr_a,all_symbols)
...
new_expressions = func.replace_coeff_with_vector(wf_equations,all_symbols)
final_expressions_wf = func.replace_params_with_vector(new_expressions,param_list)

# replace coefficients and model parameters with vector elements (for jacobian)
meson_equations = equations_s + equations_v + equations_b + equations_a
new_expressions = func.replace_coeff_with_vector(meson_equations,all_symbols)
final_expressions_meson = func.replace_params_with_vector(new_expressions,param_list)

# replace coefficients and model parameters with vector elements
en_equations = equations_en_n + equations_en_p
new_en_expr = func.replace_coeff_with_vector(en_equations,all_symbols)
...
with open(f"{A},{Z}/equations.txt", 'a') as file:
    for j in range(num_basis_states_s):
        file.write(f"    y[{n_wf_equations + j}] = pow(params[0]/enscale_mev,2.0)*S_s[{j}] - dsdr2_s[{j}] - 2.0*pow(1.0/conv_r0_en,2.0)*dsdr_s[{j}] - params[1]*(sdensn_s[{j}] + sdensp_s[{j}] - 0.5*params[4]/enscale_mev*S2_s[{j}] - 1.0/6.0*params[5]*S3_s[{j}] ); \n")
    
    for j in range(num_basis_states_v):
        file.write(f"    y[{n_wf_equations + num_basis_states_s + j}] = pow(782.5/enscale_mev,2.0)*V_v[{j}] - dvdr2_v[{j}] - 2.0*pow(1.0/conv_r0_en,2.0)*dvdr_v[{j}] - params[2]*(vdensn_v[{j}] + vdensp_v[{j}] - 1.0/6.0*params[6]*V3_v[{j}] - 2.0*params[7]*VB2_v[{j}] ); \n")
    
    for j in range(num_basis_states_b):
        file.write(f"    y[{n_wf_equations + num_basis_states_s + num_basis_states_v + j}] = pow(763.0/enscale_mev,2.0)*B_b[{j}] - dbdr2_b[{j}] - 2.0*pow(1.0/conv_r0_en,2.0)*dbdr_b[{j}] - params[3]*(0.5*vdensp_b[{j}] - 0.5*vdensn_b[{j}] - 2.0*params[7]*BV2_b[{j}] ); \n")
    
    for j in range(num_basis_states_a):
        file.write(f"    y[{n_wf_equations + num_basis_states_s + num_basis_states_v + num_basis_states_b + j}] = dadr2_a[{j}] + 2.0*dadr_a[{j}] + 1.09092217*vdensp_a[{j}]; \n")
