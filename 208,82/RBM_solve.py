...
actual_results = func.load_data(dir + f"/{A},{Z}Observables_gold.txt")
...
for i in range(nruns):
    ...
    solution = root(c_function_wrapper, x0=initial_guess_array, args=(params_array,), jac=compute_jacobian_wrapper, method='lm',options={'col_deriv': 1, 'xtol': 1e-8})
    print(solution.x)
...
rvec = np.loadtxt("208,82/High_fidelity_sol/Fn.txt")[:,0] #10,21
#actual = np.loadtxt("208,82/208,82,Data/neutron/f_wave/state3s1;2.txt")
#wave = np.loadtxt("208,82/High_fidelity_sol/Ap.txt")
meson = np.loadtxt("208,82/High_fidelity_sol/meson_fields.txt")/enscale_mev
plt.plot(r_vec,s_field_approx,ls='dashed') #
