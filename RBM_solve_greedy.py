...
param_set_gar = func.load_data("param_sets_gar.txt")
param_set_gold = func.load_data("param_sets_gold.txt")
...
###############################################################
real_en_n = np.loadtxt(f"{A},{Z}/{A},{Z}energies_n_gar.txt")
real_en_p = np.loadtxt(f"{A},{Z}/{A},{Z}energies_p_gar.txt")
...
while(finalerr>0.01):
    ...
    for s in range(len(greedy_add)):
        ...
        for i in range(len(greedy_add[s])):
            ...
            err_n = [element/50.0 for element in err_n]
            err_p = [element/50.0 for element in err_p]
